...
def index():
    if request.method == 'POST':
        ...
        time_s = float(request.form['time_s'])
        time_e = time.time()

        speed = speed_time(time_s, time_e, userinput)
        errors = mistake(original_text, userinput)

        return render_template('result.html', speed=speed, errors=errors, original_text=original_text)

    test_text = rd.choice(texts)
